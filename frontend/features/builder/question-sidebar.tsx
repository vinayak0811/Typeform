...
import { useCreateQuestion, useDeleteQuestion, useDuplicateQuestion, useReorderQuestions } from "@/hooks/use-questions";
...
export function QuestionSidebar({ form }: { form: FormDetail }) {
  ...
  function handleAdd(type: QuestionType) {
    ...
    createQuestion.mutate(
      ...
      {
        onSuccess: (question) => selectQuestion(question.id),
      }
    );
  }
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    reorderQuestions.mutate(reordered.map((q, idx) => ({ id: q.id, order: idx })));
  }

  function handleDelete(questionId: string) {
    deleteQuestion.mutate(questionId, {
      onSuccess: () => {
        if (selectedQuestionId === questionId) selectQuestion(null);
      },
    });
  }
